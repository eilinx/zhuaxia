...
class Collection(object):
    ...
    def __init__(self, xm_obj, url):
        ...
        self.xm = xm_obj
        # user id in url
        self.collection_id = re.search(
            r'(?<=/showcollect/id/)\d+', self.url).group(0)
        self.songs = []
        self.init_collection()

    def init_collection(self):
        j = self.xm.read_link(
            url_collection % (self.collection_id)).json()['collect']
...
class Xiami(object):
    ...
    def __init__(self, email, password, is_hq=False):
        ...
        if self.skip_login:
            ...
            if self.login():
                LOG.info(u'[Login] 用户: %s (id:%s) 登录成功.' %
    ...
    def login(self):
        ...
        try:
            ...
            self.uid, self.user_name = urllib.unquote(
                sess.cookies['user']).split('"')[0:2]
    ...
    def read_link(self, link):
        headers = {'User-Agent': AGENT}
        headers['Referer'] = 'http://img.xiami.com/static/swf/seiya/player.swf?v=%s' % str(
            time.time()).replace('.', '')
        proxies = None
    ...
    def decode_xiami_link(self, mess):
        ...
        len_url = len(url)
        cols = len_url / rows
        # how many rows need to extend 1 col for the remainder
        re_col = len_url % rows
